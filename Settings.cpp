...
template<>
int32_t SettingImpl<uint32_t>::setVal(const char* valStr) {
  char* endptr;
  uint32_t tempValue;
  if (!editable) {
    return -1;
...
template<>
int32_t SettingImpl<int32_t>::setVal(const char* valStr) {
  char* endptr;
  int32_t tempValue;
  errno = 0;  // To distinguish success/failure after call
...
template<>
int32_t SettingImpl<float>::setVal(const char* valStr) {
  char* endptr;
  float tempValue;
  errno = 0;  // To distinguish success/failure after call
...
Setting* Settings::getSetting(const char* name) {
  for (auto i = settings.begin(); i != settings.end(); i++) {
    if (strncmp((*i)->settingName.c_str(), name, (*i)->settingName.length()) == 0) {
      return *i;
    }
  }
  return 0;
